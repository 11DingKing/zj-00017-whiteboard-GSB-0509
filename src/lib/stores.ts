...
function createStore() {
  ...
  function canMergeUpdate(prev: Operation, next: Operation): boolean {
    if (prev.type !== "update" || next.type !== "update") return false;
    if (next.timestamp - prev.timestamp > 200) return false;
    if (prev.shapes.length !== next.shapes.length) return false;
    const prevIds = new Set(prev.shapes.map((s) => s.id));
    if (!next.shapes.every((s) => prevIds.has(s.id))) return false;
    return true;
  }
  ...
  function pushUndo(op: Operation) {
    ...
    undoStack.update((stack) => {
      if (stack.length > 0 && canMergeUpdate(stack[stack.length - 1], op)) {
        const merged = {
