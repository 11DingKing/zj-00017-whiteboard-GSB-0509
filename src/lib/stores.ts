...
function createStore() {
  ...
  let pendingUpdateOp: Operation | null = null;
  ...
  function flushPendingUpdate() {
    ...
    if (pendingUpdateOp) {
      pushUndoImmediate(pendingUpdateOp);
      pendingUpdateOp = null;
    }
  }

  function mergeUpdateOps(existing: Operation, incoming: Operation): Operation {
    const idMap = new Map<string, Shape>();
    const prevMap = new Map<string, Shape>();
    for (const s of existing.shapes) idMap.set(s.id, s);
    if (existing.previousShapes) {
      for (const s of existing.previousShapes) prevMap.set(s.id, s);
    }
    for (const s of incoming.shapes) idMap.set(s.id, s);
    if (incoming.previousShapes) {
      for (const s of incoming.previousShapes) {
    ...
    return {
      id: existing.id,
      type: "update",
      shapes: Array.from(idMap.values()),
      previousShapes: Array.from(prevMap.values()),
      userId: existing.userId,
      timestamp: Date.now(),
    };
  }

  function pushUndoImmediate(op: Operation) {
  ...
  function pushUndo(op: Operation) {
    ...
    if (op.type === "update") {
      ...
      if (pendingUpdateOp) {
        pendingUpdateOp = mergeUpdateOps(pendingUpdateOp, op);
      } else {
        pendingUpdateOp = { ...op };
      }
