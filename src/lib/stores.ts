...
import type { ToolType, Shape, Style, Operation, UserCursor, Project } from './types';
...
function createStore() {
  ...
  const selectedShapes = derived([shapes, selectedIds], ([$shapes, $selectedIds]) => {
  ...
  function updateShape(id: string, updates: Partial<Shape>, broadcast: boolean = true) {
    ...
    shapes.update(s => s.map(item => item.id === id ? { ...item, ...updates, updatedAt: Date.now(), timestamp: Date.now() } : item));
    if (broadcast) {
      pushUndo(op);
      broadcastOperation(op);
    }
  ...
  function updateShapes(updates: Array<{ id: string; updates: Partial<Shape> }>, broadcast: boolean = true) {
    ...
    for (const { id, updates: u } of updates) {
      ...
      if (current) {
        ...
        newShapes.push({ ...current, ...u, updatedAt: Date.now(), timestamp: Date.now() });
      }
    }
    ...
    shapes.update(s => s.map(item => {
      const u = idMap.get(item.id);
      if (u) {
        return { ...item, ...u, updatedAt: Date.now(), timestamp: Date.now() };
      }
      return item;
    }));
    if (broadcast) {
  ...
  function execute(op: Operation) {
    switch (op.type) {
      case 'add':
        ...
        break;
      case 'remove':
        removeShapes(op.shapes.map(s => s.id), false);
        break;
      case 'update':
  ...
  function receiveOperation(op: Operation) {
    ...
    if (existing && op.type === 'update') {
      ...
      if (existingShape && op.timestamp > existingShape.timestamp) {
        shapes.update(s => s.map(item => item.id === op.shapes[0].id ? { ...op.shapes[0], updatedAt: Date.now() } : item));
      }
    }
  }

  function setZIndex(ids: string[], direction: 'front' | 'back' | 'up' | 'down') {
    const $shapes = get(shapes);
